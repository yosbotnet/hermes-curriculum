...
3. **Export your API key (and any optional overrides).**
   ...
   | `CURRICULUM_MAX_TOKENS`   | `16384`                                                        | Completion budget per LLM call. Reasoning models spend this same budget on hidden thinking BEFORE any visible output; if the build log shows "EMPTY completion" warnings, raise this (e.g. 32768) or use a non-reasoning ingest model. |
