"""Nous-backed LlmProvider -- the only place paid inference happens.

OpenAI-compatible ``/chat/completions`` over the standard library ``urllib``
(no extra dependency). The ingestion passes use this in a real run; tests and
dry runs use :class:`curriculum.providers_fake.FakeLlm` instead.
    ...
    def complete(
        ...
        *,
        ...
        try:
            ...
            print(f"[NousLlm] HTTP {exc.code}: {detail}", file=sys.stderr)
            return ""
        except (urllib.error.URLError, TimeoutError, OSError) as exc:  # pragma: no cover
            self.failures += 1
            print(f"[NousLlm] request failed: {exc}", file=sys.stderr)
            return ""
        try:
            return payload["choices"][0]["message"].get("content") or ""
        except (KeyError, IndexError, TypeError):  # pragma: no cover
            self.failures += 1
            print(f"[NousLlm] unexpected response shape: {str(payload)[:200]}", file=sys.stderr)
            return ""


class NousEmbedder(EmbeddingProvider):
    """Nous-backed embeddings (OpenAI-compatible /embeddings), batched.
