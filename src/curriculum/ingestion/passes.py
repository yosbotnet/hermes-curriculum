...
class InferEdgesPass(IngestionPass):
    ...
    def run(self, ctx: IngestionContext) -> None:
        ...
        # Spine-heads: concepts ON the spine with no incoming spine
        # PREREQUISITE -- the entry points of the trusted chain. Inference must
        # not create PREREQUISITE edges targeting a head (that would close an
        # entry into the graph). ONLY heads are protected: off-spine satellite
        # concepts remain valid targets, because inferred prereqs into them are
        # exactly the cross-links this pass exists to add.
        spine_heads = {src for src, _ in spine_keys} - {dst for _, dst in spine_keys}
        ...
        for i in range(0, len(concepts), bs):
            self._infer(ctx, concepts[i : i + bs], concepts, files, spine_keys, spine_heads)
    ...
    def _infer(
        ...
        for item in _items(raw, "edges"):
            ...
            if edge.type is EdgeType.PREREQUISITE and edge.dst in spine_heads:
                continue  # never close an entry point of the trusted chain
